...
def xgbost_train(features_df, results_df, model_name):
    ...
    # QC: Plot histogram of median K values (still continuous)
    plt.figure()
    ...
    plt.close()
    print(f"Median K value histogram saved as {model_name + '_median_K_histogram.png'}")
    print("Median K value variance:", np.var(y_raw))
    print("Median K value mode:", pd.Series(y_raw).mode().values)
